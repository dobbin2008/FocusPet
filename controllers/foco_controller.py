...
from services.foco_service import FocoService
...
class FocoController:
    ...
    def iniciar_modo_foco(self):
        ...
        try:
            sessao = self.service.iniciar(session["user_id"])
    ...
    def encerrar_modo_foco(self, sessao_id):
        if "user_id" not in session:
            return jsonify({"erro": "Não autenticado"}), 401
        try:
            sessao = self.service.encerrar(session["user_id"], sessao_id)
            return jsonify({
                "sucesso": True,
    ...
    def obter_status_sessao(self, sessao_id):
        if "user_id" not in session:
            return jsonify({"erro": "Não autenticado"}), 401
        try:
            sessao = self.service.buscar_do_estudante(session["user_id"], sessao_id)
            return jsonify({
                "sucesso": True,
                "sessao_id": sessao.id,
    ...
    def registrar_distracao(self, sessao_id):
        if "user_id" not in session:
            return jsonify({"erro": "Não autenticado"}), 401
        try:
            sessao = self.service.registrar_distraicao(session["user_id"], sessao_id)
...
foco_controller = FocoController(FocoService())
