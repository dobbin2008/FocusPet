...
class BlacklistController:
    ...
    def adicionar_site_blacklist(self):
        ...
        try:
            site = self.service.adicionar(session["user_id"], request.get_json(silent=True) or {})
    ...
    def remover_site_blacklist(self, site_id):
        if "user_id" not in session:
            return jsonify({"erro": "Não autenticado"}), 401
        try:
            url = self.service.remover(session["user_id"], site_id)
    ...
    def listar_blacklist(self):
        if "user_id" not in session:
            return jsonify({"erro": "Não autenticado"}), 401
        try:
            sites = self.service.listar(session["user_id"])
    ...
    def obter_urls_bloqueadas(self):
        if "user_id" not in session:
            return jsonify({"erro": "Não autenticado"}), 401
        try:
            return jsonify({"sucesso": True, "urls_bloqueadas": self.service.urls(session["user_id"])}), 200
        except Exception as erro:
            return jsonify({"erro": str(erro)}), 500


blacklist_controller = BlacklistController(BlacklistService())
